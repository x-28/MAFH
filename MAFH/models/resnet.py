...
class ResNet(BasicModule):
    ...
    def __init__(self, bit, loss, block, layers,
                 last_stride=2,
                 fc_dims=None,
                 dropout_p=None,
                 **kwargs):
        ...
        self.classifier = nn.Linear(self.feature_dim, bit)
...
def resnet18(bit, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
        bit=bit,
        loss=loss,
        block=BasicBlock,
...
def resnet34(bit, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
        bit = bit,
        loss=loss,
        block=BasicBlock,
...
# model= resnet18(bit=32)
# #model=vit_small_patch16_224(pretrained=True,bit=16)
# model.cuda()
# out = model(x.cuda())
# print(out.shape)