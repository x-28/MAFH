...
from loss.v0multisimilarity import multilabelsimilarityloss_KL
...
def train(dataset_name: str, bit: int, issave=True, batch_size=128, use_gpu=True, max_epoch=150, lr=10 ** (-1.5), isvalid=True, alpha=0.3, beta=1.9, gamma=0.9):
    ...
    if mapt2i + mapi2t >= max_mapi2t + max_mapt2i:
        img_model.save_dict(
            os.path.join("D:/lixue/cross-modal/20220812DCMH/MESDCH-master/pth/MESDCH", str(bit) + '-' + img_model.module_name + '.pth'))
        txt_model.save_dict(
            os.path.join("D:/lixue/cross-modal/20220812DCMH/MESDCH-master/pth/MESDCH", str(bit) + '-' + txt_model.module_name + '.pth'))