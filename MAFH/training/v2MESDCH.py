...
from loss.v2multisimilarity import  multilabelsimilarityloss_KL1, multilabelsimilarityloss_KL,remark_loss,reset_loss,bit_scalable
from loss.quantizationloss import quantizationLoss
from utils.save_results import save_hashcodes
from loguru import logger

def train(dataset_name: str, bit: int, issave=True, batch_size=128, use_gpu=True, max_epoch=150, lr=10 ** (-1.5), isvalid=True, alpha=0.3, beta=1.9, gamma=0.9):
    ...
    for epoch in range(max_epoch):
        ...
        for data in tqdm(train_loader):  # tqdm用来显示进度条的
            ...
            KLloss_xx = beta * multilabelsimilarityloss_KL1(sample_L, train_L, hash, F)
            KLloss_xl = multilabelsimilarityloss_KL1(sample_L, train_L, hash, L)
            quantization_x = gamma * quantizationLoss(hash, B[ind, :])
        ...
        for data in tqdm(train_loader):  # tqdm用来显示进度条的
            ...
            KLloss_yy = beta * multilabelsimilarityloss_KL1(sample_L, train_L, hash, G)
            KLloss_yl = multilabelsimilarityloss_KL1(sample_L, train_L, hash, L)
            #yx =
            quantization_y = gamma * quantizationLoss(hash, B[ind, :])
        ...
        if isvalid:
            ...
            if mapt2i + mapi2t >= max_mapt2i + max_mapi2t:
                ...
                max_mapt2i = mapt2i
                best_epoch = epoch+1
            print('epoch: %1d/1d,valid MAP: %3.4f,MAP(i->t): %3.4f, MAP(t->i): %3.4f;MAX_MAP(i->t): %3.4f, MAX_MAP(t->i): %3.4f,best_epoch:%1d' % (epoch + 1, max_epoch,mapi2t, mapt2i, max_mapi2t, max_mapt2i,best_epoch))
...
#     logger.info(str(bit)+'max_mapi2t: {:.4f}'.format(max_mapi2t)+'max_mapi2t: {:.4f}'.format(max_mapt2i))
    ...
    if mapt2i + mapi2t >= max_mapi2t + max_mapt2i:
        ...
        best_epoch = epoch
        img_model.save_dict(
            os.path.join("D:/lixue/cross-modal/20220812DCMH/MESDCH-master/pth", str(bit) + '-' + img_model.module_name + '.pth'))
        txt_model.save_dict(
            os.path.join("D:/lixue/cross-modal/20220812DCMH/MESDCH-master/pth", str(bit) + '-' + txt_model.module_name + '.pth'))
        torch.save(label_model.state_dict(),os.path.join("D:/lixue/cross-modal/20220812DCMH/MESDCH-master/pth", str(bit) + '-' + 'label' + '.pth'))
        # label_model.save_dict(
        #     os.path.join("D:/lixue/cross-modal/20220812DCMH/MESDCH-master/pth", str(bit) + '-' + label_model.module_name + '.pth'))
        # qB_img = qB_img.cpu()
        # qB_txt = qB_txt.cpu()
        # rB_img = rB_img.cpu()
        # rB_txt = rB_txt.cpu()
