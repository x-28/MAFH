...
def save_hashcodes(batch_size, use_gpu, bit, img_model, txt_model, dataset_name, valid_data, method_name):
    ...
    qB_img, qB_txt, rB_img, rB_txt, queryLabel, retrievalLabel = valid(batch_size, bit, use_gpu, img_model, txt_model,
                                                                                       valid_data, return_hash=True)
    mAPi2t, mAPt2i= valid(batch_size, bit, use_gpu, img_model, txt_model,valid_data, return_hash=False)
    ...
    result_dir = "D:/lixue/cross-modal/20220812DCMH/MESDCH-master/result_hashcodes"
    sio.savemat(
